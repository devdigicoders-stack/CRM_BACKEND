...
export const createUser = async (req, res, next) => {
  try {
    ...
    const isUserRole = ['accountant', 'sales', 'calling', 'telecaller', 'installation', 'crmuser', 'stock'].includes(targetRole);

    if (!isAdminRole && !isUserRole) {
      res.status(400);
      throw new Error('Invalid role. Allowed roles: admin, superAdmin, accountant, sales, calling, telecaller, installation, crmuser, stock');
    }
